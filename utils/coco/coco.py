...
class COCO:
    ...
    def createIndex(self):
        ...
        if 'images' in self.dataset:
            imgs      = {im.split('.')[0]: {} for im in self.dataset['images']}
            for img in self.dataset['images']:
                imgs[img.split('.')[0]] = img
                img_name_to_id[img.split('.')[0]] = img.split('.')[0]
    ...
    def customLoadRes(self, resFile, captions, dataset):
      res = COCO()
      res.dataset['images'] = [captions.iloc[i, 0] for i in range(len(captions))]
      print('Loading and preparing results...     ')
      tic = time.time()
      anns = json.load(open(resFile))
      assert type(anns) == list, 'results in not an array of objects'
      annsImgIds = [ann['image_id'] for ann in anns]
      assert set(annsImgIds) == (set(annsImgIds) & set(dataset.image_ids)), \
              'Results do not correspond to current coco set'
      assert 'caption' in anns[0]
      imgIds = set([img.split('.')[0] for img in res.dataset['images']]) & set([ann['image_id'] for ann in anns])
      res.dataset['images'] = [img for img in res.dataset['images'] if img.split('.')[0] in imgIds]
