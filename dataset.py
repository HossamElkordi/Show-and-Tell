...
def prepare_eval_data(config):
    ...
    # image_files = [os.path.join(config.eval_image_dir,
                                # coco.imgs[image_id]['file_name'])
                                # for image_id in image_ids]
    image_ids = list(set([captions.iloc[i, 0].split('.')[0] for i in range(len(captions))]))
    image_files = list(set([os.path.join(config.eval_image_dir, captions.iloc[i, 0]) for i in range(len(captions))]))
    ...
    print("Dataset built.")
    return captions, dataset, vocabulary
...
def build_vocabulary(config):
    ...
    captions = pd.read_csv(config.eval_caption_file, header = None)

    vocabulary = Vocabulary(config.vocabulary_size)
    vocabulary.build([captions.iloc[i, 1] for i in range(len(captions))])
    # vocabulary.build(coco.all_captions())
    vocabulary.save(config.vocabulary_file)
    return vocabulary
